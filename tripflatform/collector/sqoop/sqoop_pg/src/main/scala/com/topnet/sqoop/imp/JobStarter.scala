...
import com.topnet.sqoop.SQClient
...
//  ----导出company表
  ...
  def main(args: Array[String]): Unit = {
   /* val import_companyJob = companyJob()
    doJob(import_companyJob)

    val import_hotelJob = hotelJob()
    doJob(import_hotelJob)*/
/*
    val import_buildingJob = buildingJob()
    doJob(import_buildingJob)

    val import_layersJob = layersJob()
    doJob(import_layersJob)

    val import_roominfoJob = roominfoJob()
    doJob(import_roominfoJob)

    val import_roomTypeJob = roomTypeJob()
    doJob(import_roomTypeJob)

    val import_tbCustomerJob = tbCustomerJob()
    doJob(import_tbCustomerJob)

    val import_tbCustSourceJob = tbCustSourceJob()
    doJob(import_tbCustSourceJob)*/

    val import_tbBookingJob = tbBookingJob()
    doJob(import_tbBookingJob)

    /*val import_tbBookedroomJob = tbBookedroomJob()
    doJob(import_tbBookedroomJob)*/
